...
// texture into the object's bounds like any other image. The renderer runs with
// the painter's GL context current on the render thread, which is exactly the
// contract libmpv's OpenGL Render API requires.
func (p *painter) drawGLVideo(v *canvas.GLVideo, pos fyne.Position, frame fyne.Size) {
	...
	// platform (macOS notably uses a non-zero default).
	var prevFBO int32
